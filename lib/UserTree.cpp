...
void UserTree::insertUser(const std::string& name, const std::string& email) {
    if (m_root == nullptr) {
        m_root = new BSTUserNode(User(ResourceManager::nextUserId(), name, email));
    } else {
        insertUserRecursive(m_root, name, email);
    }
}

void UserTree::insertUser(const User& user) {
    if (m_root == nullptr) {
...
// Recursive Methods
void UserTree::insertUserRecursive(BSTUserNode* node, const std::string& name, const std::string& email) {
    if (Formatter::toLower(email) < Formatter::toLower(node->user.getEmailAddress())) {
        // Insert in the left subtree
        if (node->left == nullptr) {
            node->left = new BSTUserNode(User(ResourceManager::nextUserId(), name, email));
        } else {
            insertUserRecursive(node->left, name, email);
        }
    } else {
        // Insert in the right subtree
        if (node->right == nullptr) {
            node->right = new BSTUserNode(User(ResourceManager::nextUserId(), name, email));
        } else {
            insertUserRecursive(node->right, name, email);
        }
    }
}

void UserTree::insertUserRecursive(BSTUserNode* node, const User& user) {
